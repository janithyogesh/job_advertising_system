...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleMaxUploadSizeExceeded(
        ...
        return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
                .body(new ApiErrorResponse("Uploaded file exceeds the allowed size", HttpStatus.PAYLOAD_TOO_LARGE.value()));
    }

    @ExceptionHandler(MissingServletRequestPartException.class)
    public ResponseEntity<ApiErrorResponse> handleMissingServletRequestPart(
            MissingServletRequestPartException exception) {
        String message = "Missing required file part: " + exception.getRequestPartName();
    ...
    public ResponseEntity<ApiErrorResponse> handleMultipartException(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ApiErrorResponse("Invalid multipart request", HttpStatus.BAD_REQUEST.value()));
    }
